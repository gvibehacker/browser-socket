 ...
 * @param payload - Uint8Array containing 3-byte window size + address string
 * @returns Tuple of [windowSize, AddressInfo]
 * @throws {Error} When payload format is invalid
 * @example
 * ```javascript
 * // Parse SYN payload with window size
 * const windowBytes = new Uint8Array([0x00, 0x10, 0x00]);
 * const addressBytes = new TextEncoder().encode('localhost:3000');
 * const payload = new Uint8Array([...windowBytes, ...addressBytes]);
 ...
 * const windowBytes2 = new Uint8Array([0x00, 0x20, 0x00]);
 * const addressBytes2 = new TextEncoder().encode('[::1]:8080');
 * const payload2 = new Uint8Array([...windowBytes2, ...addressBytes2]);
 * const [windowSize2, addressInfo2] = parseSynPayload(payload2);
 * // Returns: [8192, { type: 'tcp', host: '::1', port: 8080 }]
 * ```
 */
export function parseSynPayload(payload: Uint8Array): [number, AddressInfo] {
  const windowSize = (payload[0] << 16) | (payload[1] << 8) | payload[2];

  // Extract address string from remaining payload
  const addressStr = new TextDecoder().decode(payload.slice(3));
